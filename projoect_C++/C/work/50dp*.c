...
int main() {
    ...
    while (scanf("%d%d%d", &m, &s, &t) == 3) {
        ...
        for (i = 1; i <= t; i++) 
        {
            if (m >= 10) 
            {
                // If there's enough magic, use blink
                dp2[i] = dp2[i - 1] + 60;
                m -= 10;
            } 
            else 
            {
                // If not enough magic for blink, restore magic and wait
                m += 4;
                dp2[i] = dp2[i - 1];
            }
            //dp2数组存储只用blink时所能到的距离
            //dp1数组存储在当前时间内能到的最远距离
            //状态转移方程
            dp1[i] = (dp1[i - 1] + 17) > dp2[i] ? (dp1[i - 1] + 17) : dp2[i]; // Choose maximum
            if (dp1[i] >= s) 
            {
                break;
            }
